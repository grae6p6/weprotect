				...
				<div class="row has-blocks">
					<?php foreach ( $posts as $i => $post ) :
						...
						<div class="col-12 col-md-<?php echo $col_md ?>">
							...
							<?php else : ?>
								<div class="block light" id="block-wl-<?php echo $the_ID ?>-<?php echo $i ?>"><a
											href="<?php the_permalink() ?>"><h5><?php the_title() ?></h5>
									</a><span><?php the_time( get_option( 'date_format' ) ) ?></span></div>
							<?php endif; ?>
						</div>
					<?php endforeach;
					wp_reset_postdata(); ?>
						...
						<div class="btn-group" data-toggle="buttons">
							<label class="btn btn-primary active" data-group="country">
								<input type="radio" name="options"
									   autocomplete="off"<?php echo( $group == 'country' ? ' checked' : '' ) ?>><i
										class="fa fa-filter" aria-hidden="true"></i> <?php _e( 'Countries', 'wep' ) ?>
							</label>
							<label class="btn btn-primary" data-group="organisation">
								<input type="radio" name="options"
									   autocomplete="off"<?php echo( $group == 'organisation' ? ' checked' : '' ) ?>><i
										class="fa fa-filter"
										aria-hidden="true"></i> <?php _e( 'Organisations', 'wep' ) ?>
							</label>
							<label class="btn btn-primary" data-group="industry">
								<input type="radio" name="options"
									   autocomplete="off"<?php echo( $group == 'industry' ? ' checked' : '' ) ?>><i
										class="fa fa-filter" aria-hidden="true"></i> <?php _e( 'Companies', 'wep' ) ?>
							</label>
							<label class="btn btn-primary" data-group="">
								<input type="radio" name="options"
									   autocomplete="off"<?php echo( $group == '' ? ' checked' : '' ) ?>><i
										class="fa fa-globe" aria-hidden="true"></i> <?php _e( 'All', 'wep' ) ?>
							</label>
						</div>
					</div>
					<div class="col-12 col-lg-4 text-center text-lg-right buttons">
						...
						<div class="btn-group" data-toggle="buttons">
							<label class="btn btn-primary" data-display="map">
								<input type="radio" name="options" autocomplete="off"><i class="fa fa-map"
																						 aria-hidden="true"></i> <?php _e( 'Map', 'wep' ) ?>
							</label>
							<label class="btn btn-primary active" data-display="list">
								<input type="radio" name="options" autocomplete="off" checked><i class="fa fa-th"
																								 aria-hidden="true"></i> <?php _e( 'List', 'wep' ) ?>
							</label>
						</div>
					</div>
				</div>
					...
					<div class="modal-content">
						<div class="modal-header">
							<h5 class="modal-title"
								id="memberModalLabel"><?php _e( 'Member information', 'wep' ) ?></h5>
							<button type="button" class="close" data-dismiss="modal"
									aria-label="<?php _e( 'Close', 'wep' ) ?>">
			...
			<div class="flex-container">
				<div class="row members list">
					<?php foreach ( $members as $member ) :
						...
						?>
						<div class="col-6 col-sm-4 col-md-3 entry <?php echo get_field( 'group', $member->ID ) ?>"
							 data-code="board-<?php echo $member->ID ?>">
							<a href="javascript:void(0)" data-toggle="modal" data-target="#memberModal">
								<div>
									<?php if ( has_post_thumbnail( $member->ID ) ) : ?>
										<img src="<?php echo get_the_post_thumbnail_url( $member->ID, 'block-cta' ) ?>"
						...
						<?php foreach ( $entries as $link ) : ?>
							<?php if ( $link->link_name ) : /* Link */ ?>
								...
								<div class="col-8 col-md-9">
									<h5><a href="<?php echo $link->link_url ?>"
										   target="_blank"><?php echo $link->link_name ?></a></h5>
									<p>
										<span class="posted-on"><strong><?php echo parse_url( $link->link_url, PHP_URL_HOST ) ?></strong>&nbsp;<i
													class="fa fa-external-link"
													aria-hidden="true"></i><br><?php echo date( get_option( 'date_format' ), strtotime( $link->link_updated ) ) ?></span>
									</p>
