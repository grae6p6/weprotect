						...
						<div class="row">
											...
											<div class="row align-items-center">
												...
												<div class="col text <?php echo( ! $i ? '' : 'px-0' ) ?>">
													<a href="<?php echo get_the_permalink( $group->ID ); ?>"
													   alt="<?php echo get_the_title( $group->ID ); ?>">
														<?php echo( ! $i ? __( 'Introduction', 'wep' ) : get_the_title( $group->ID ) ); ?>
													</a>
												</div>
												<?php if ( $i > 0 ) : ########################### ?>
													<div class="col p-0 pr-3">
														<a href="javascript:void(0)" class="toggle float-right"
														   aria-label="<?php _e( 'Menu for ' . get_the_title( $group->ID ), 'wep' ); ?>"><i
							...
							<div class="col-12 col-md-8 col-lg-9 content">
								...
								<?php else: the_title( '<h1>', '</h1>' ); ?>
									...
									<?php endif; ?>
									<div class="outcome"><h5><?php _e( 'Outcomes', 'wep' ) ?></h5><?php the_content() ?>
									</div>
									<?php
									...
									if ( $sections ) :
										foreach ( $sections as $post ) : setup_postdata( $post ); ?>
											<section
													id="<?php echo $post->post_name ?>"><?php the_content() ?></section>
											<?php wp_reset_postdata(); endforeach;
									endif;
								endif; ?>
							</div>
